...
@synthesize cmdText = __cmdText;
@synthesize logView = __logView;
@synthesize systemView = __systemView;
@synthesize tabView = __tabView;
...
{
    [[NDAppDelegate instance]
     log:msg file:file line:line method:method
     ];
}
...
- (IBAction)saveAction:(id)sender
{
    ...
    if (![[self managedObjectContext] commitEditing]) {
        NDLog(@"%@:%@ unable to commit editing before saving", [self class], NSStringFromSelector(_cmd));
    }
...
{
    // Should be factored out into Cmd class
	NSString *cmdString	= [sender stringValue];
	[sender setStringValue:@""];
    
    [self.commanding execute: cmdString];
}



// Returns the directory the application uses to store the Core Data store file. This code uses a directory named "Folkwang-Enterprises.Temperance" in the user's Application Support directory.
- (NSURL *)applicationFilesDirectory
{
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSURL *appSupportURL = [[fileManager URLsForDirectory:NSApplicationSupportDirectory inDomains:NSUserDomainMask] lastObject];
    return [appSupportURL URLByAppendingPathComponent:@"Folkwang-Enterprises.Temperance"];
}
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"Temperance" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    if (!mom) {
        NDLog(@"%@:%@ No model to generate a store from", [self class], NSStringFromSelector(_cmd));
        return nil;
    }
    ...
    NSURL *url = [applicationFilesDirectory URLByAppendingPathComponent:@"Temperance.storedata"];
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication *)sender
{
    ...
    if (![[self managedObjectContext] commitEditing]) {
        NDLog(@"%@:%@ unable to commit editing to terminate", [self class], NSStringFromSelector(_cmd));
